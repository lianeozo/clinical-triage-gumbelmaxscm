...
class DataGenerator(object):
    ...
    def simulate(self, num_iters, max_num_steps,
        ...
        iter_states = np.ones((num_iters, max_num_steps+1, 11), dtype=int)*(-1)
        ...
        for itr in tqdm(range(num_iters), disable=not(use_tqdm), desc=tqdm_desc):
            ...
            iter_states[itr, 0] = mdp.state.get_state_vector() # store state features (change from getting ID)
            for step in range(max_num_steps):
                ...
                iter_states[itr,step+1] = mdp.state.get_state_vector()
